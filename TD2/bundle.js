...
angular.module("mainApp").controller("RouteController",["$scope","$location","$window","run","pathCtrl",require("./controllers/mainController")]);
angular.module("mainApp").factory("pathCtrl",function(){
    return {ex:"/ex1",run:"/ex3"};
}).run(['$rootScope', '$location','pathCtrl',function($rootScope,$location,pathCtrl){
    $rootScope.$on("$locationChangeStart", function (event) {
        var path=$location.path();
        var comp=pathCtrl.run;
        if(path!="/home"){
            if(path.indexOf("/run")==0)
            comp=pathCtrl.ex;
...
},{"./controllers/mainController":2,"./directives/drag":3,"./directives/forwardLink":5,"./directives/modal":6,"./directives/progressBar":7,"./ex1-note/app":8,"./ex2-services/app":10,"./ex3-choixMultiples/app":11,"./ex4-contacts/app/app":12,"./ex5-currency/app/app":17,"./ex6-calc/app":22,"./ex7-button-dir/app/app":26,"./ex8-alert-dir/app/app":29,"./ex9-pb-dir/app/app":30,"./filters/truncate":32,"./prismApp/app":33,"./routes":39}],2:[function(require,module,exports){
var mainController=function($scope,$location,$window,run,pathCtrl){
	$scope.vousEtesIci=function(){
		return $location.path().replace("/","").replace("run/","");
	}
	$scope.notPermise=function(num){
		var str= "/ex"+num;
		return pathCtrl.ex<str && pathCtrl.run<str;
	};
	
	$scope.href=function(num){
		var str= "/ex"+num;
		var href="ex"+num;
		$onlyEx= pathCtrl.ex<str && pathCtrl.run>=str;
		if(!$onlyEx)
			href="run/"+href;
		return href;
	};
	
	$scope.label=function(num){
		var result="indisponible";
		var str= "/ex"+num;
		if(pathCtrl.run>=str)
			result="Exécution disponible";
		if(pathCtrl.ex>=str)
			result="Corrigé disponible";
		return result;
	};
	
	$scope.css=function(num){
		var result=0;
		var str= "/ex"+num;
		if(pathCtrl.run>=str)
			result++;
...
angular.module("NoteApp").controller("NoteController", ["$scope","$cookies",require("./controller")]);
module.exports=noteApp.name;
},{"./controller":9}],9:[function(require,module,exports){
var noteController=function($scope,$cookies) {
    ...
    $scope.save  = function() {$cookies.put("note",$scope.message);$scope.info="Note sauvegardée";};
...
angular.module("ServicesApp").controller("ServicesController", [ "$scope","$http", function($scope,http) {
	...
	http.get('js/ex2-services/promo.json').success(function(data) {
		$scope.codesPromo = data;
	});
	...
	$scope.applyCode=function(){
		$scope.remise="Code promo invalide";
		if($scope.codesPromo[$scope.codePromo]){
			$scope.remise=$scope.codesPromo[$scope.codePromo]*$scope.total();
		}
	};
...
angular.module('currencyApp').service("CurrencyService",['$http',require("./currencyService")]);
angular.module("currencyApp").controller("currencyController",['$scope','$http','$interval','CurrencyService',require("./currencyControllers")]);
...
},{"./classChangeDirective":18,"./currencyControllers":19,"./currencyService":20,"./histoElemDirective":21}],18:[function(require,module,exports){
...
var currencyController= function($scope,$http,$interval,CurrencyService) {
	$scope.historique=CurrencyService.historique;
	$scope.histo=true;
	...
	$scope.update=function(from,to,what){
		CurrencyService.update(from,to,what,$scope.histo,$scope.result);
	};
...
/**
 * Created by jc on 26/01/2016.
 */
var currencyService=function($http){
    this.historique={};
    var self=this;
    this.update=function(from,to,what,hasHisto,result){
        $http.jsonp('http://free.currencyconverterapi.com/api/v3/convert?compact=y&q='+from.code+'_'+to.code+'&callback=JSON_CALLBACK').
        success(function(data, status, headers, config) {
            val=data[from.code+'_'+to.code].val;
            result.value=val*what;
            if(hasHisto===true){
                var conversion={
                    from : from,
                    to : to,
...
app.controller("calcController", ["calcService",require("./calcController")]);
app.directive('btn', ["$compile","calcService",require("./buttonDirective")]);
app.directive('focusMe', [require("./../directives/focusMe")]);

module.exports=app;
},{"./../directives/focusMe":4,"./buttonDirective":23,"./calcController":24,"./calcService":25}],23:[function(require,module,exports){
var button=function($compile,calcService){
	return {
		restrict: 'E',
		transclude: false,
		replace:true,
		scope: { value:'@',onAdd:'&',cssClass:'@',title:'@'},
		link: function(scope, element, attrs) {
			...
			default:
				...
				element.replaceWith($compile('<input title="{{title}}" type="button" value="{{value}}" class="btn calcBtn '+cssClass+'" data-ng-click="onAdd({element: \'{{value}}\'})">')(scope));
				break;
			}
		}
	};
...
var calcController=function(CalcService) {
	this.gui=CalcService.gui;
	this.ops=CalcService.ops;
...
},{"./buttonDirective":27}],27:[function(require,module,exports){
...
},{"./alertDirective.js":28}],30:[function(require,module,exports){
...
module.exports=app.name;
},{"./pbDirective":31}],31:[function(require,module,exports){
module.exports= function() {
	return {
		restrict: "E",
		require: 'ngModel',
...
},{"./directives/prismTabs":34,"./directives/tabContent":35,"./directives/tabHeader":36,"./prismController":37,"./services/download":38}],34:[function(require,module,exports){
